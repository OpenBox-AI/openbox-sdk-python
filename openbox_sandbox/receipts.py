...
from .contracts import GovernedCommandReceipt, GovernedCommandRequest
        ...
        _canonical(
            {
                ...
                "arguments": [
                    {"name": item.name, "value": item.value} for item in request.arguments
                ],
            }
        )
    ).hexdigest()
...
def receipt_binding(
    ...
    *,
    ...
    if not isinstance(profile_fingerprint, str) or _SHA256.fullmatch(profile_fingerprint) is None:
        raise GovernedCommandReceiptError("governed command receipt rejected")
    return {
...
def _consume_receipt(
    ...
    *,
    ...
    with lock:
        if receipt.receipt_id in consumed_receipt_ids or receipt.nonce in consumed_nonces:
...
class GovernedCommandReceiptVerifier:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise GovernedCommandReceiptError(_SIGNED_ERRORS.verifier_rejected) from error
    ...
    def __repr__(self) -> str:
        return (
            f"SandboxReceiptVerifier(key_id={self.key_id!r}, "
...
class InsecureLocalReceiptVerifier:
    ...
    def verify(
        ...
        *,
        ...
        _validate_common_time_window(receipt, clock=self.clock, errors=_INSECURE_LOCAL_ERRORS)
...
@dataclass(frozen=True, slots=True)
class AuthorizedConstrain:
    """Explicit caller attestation that Core already returned ``CONSTRAIN``."""
    ...
    def __post_init__(self) -> None:
        if (
            self.verdict != "constrain"
            or not isinstance(self.authorization_id, str)
            or _IDENTIFIER.fullmatch(self.authorization_id) is None
        ):
            raise GovernedCommandReceiptError("authorized CONSTRAIN attestation required")


class ReceiptSigner(Protocol):
    """External signer boundary; implementations retain their private key."""
...
def issue_sandbox_receipt(
    request: GovernedCommandRequest,
    *,
    authorization: AuthorizedConstrain,
    ...
    """Bind one already-authorized request to the installed approved release.

    The issuer does not call Core and cannot mint a receipt without the explicit
    ``AuthorizedConstrain`` marker. The signer receives only canonical public
    receipt bytes; raw private-key material never enters this API.
    """
    from .registry import GovernedCommandRegistry
    from .release import materialize_approved_sandbox_release

    if (
        not isinstance(request, GovernedCommandRequest)
        or request.receipt is not None
        or not isinstance(authorization, AuthorizedConstrain)
        or not isinstance(registry, GovernedCommandRegistry)
        or not isinstance(workflow_id, str)
        or _WORKFLOW_ID.fullmatch(workflow_id) is None
    ...
    try:
        argv = profiles.derive(request, now=issued_at)
        ...
        unsigned = GovernedCommandReceipt(
            schema_version=1,
            ...
            asset_bundle_sha256=binding["asset_bundle_sha256"],
        ...
        signature = signer.sign(_canonical(receipt_payload(unsigned)))
    except GovernedCommandReceiptError:
        raise
    except Exception:
        raise GovernedCommandReceiptError("governed command receipt issuance rejected") from None
    ...
    return GovernedCommandRequest(request.profile_id, request.arguments, receipt)


# Public framework-neutral names.
