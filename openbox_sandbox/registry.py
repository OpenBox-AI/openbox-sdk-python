...
produces both the structured argv-derivation mapping and the dispatcher
...
class EnumArgument:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or not all(
                isinstance(item, str) and "\x00" not in item and 0 < len(item.encode()) <= 4096
...
class TypedJsonResultSchema:
    """Canonical bounded JSON output admitted into durable framework results."""
...
class GovernedCommandRegistry:
    ...
    def structured_profile_bundle(self) -> StructuredCommandProfileBundle:
        """Build the framework-neutral structured-input mapping."""
        ...
        bundle = object.__new__(StructuredCommandProfileBundle)
