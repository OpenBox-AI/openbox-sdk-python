"""Strict, callable-free structured command profile mapping."""
...
class CommandProfileBundleError(ValueError):
    def __init__(self) -> None:
        super().__init__("structured command profile bundle rejected")
...
class _ArgumentMapping:
    ...
    def parse(cls, value: object) -> "_ArgumentMapping":
        ...
        if kind == "literal":
            ...
            if not isinstance(item, str) or "\x00" in item or len(item.encode()) > 4096:
                raise CommandProfileBundleError()
        ...
        if kind == "field_enum":
            ...
            if (
                ...
                or not all(
                    ...
                    and "\x00" not in choice
                    and len(choice.encode("utf-8")) <= 4096
...
class _ResultField:
    ...
    def validate(self, value: object) -> GovernedCommandResultValue:
        if self.kind == "identifier":
            ...
            raise CommandResultValidationError()
        try:
            return GovernedCommandResultValue(self.name, value)
...
class _ResultSchema:
    ...
    def parse_output(self, output: bytes) -> GovernedCommandTypedResult:
        ...
        try:
            ...
            raise CommandResultValidationError() from None
        if not isinstance(value, dict) or set(value) != {field.name for field in self.fields}:
            raise CommandResultValidationError()
        try:
...
def _parse_profile_values(value: object) -> dict[str, _Profile]:
    ...
    for raw in value:
        if not isinstance(raw, dict):
            raise CommandProfileBundleError()
        if set(raw) == base_profile_fields and raw.get("result_mode") == "metadata_only":
            item = raw
            result_schema = None
        elif (
...
class StructuredCommandProfileBundle:
    schema_version: int
    bundle_version: str
    key_id: str
    issued_at: datetime
    ...
    def __init__(self) -> None:
        raise TypeError("use load() or from_trusted() to construct structured profiles")
    ...
    def load(
        ...
        *,
        ...
        object.__setattr__(instance, "fingerprint", hashlib.sha256(_canonical(payload)).hexdigest())
    ...
    def profile_fingerprint(self, profile_id: str, *, now: datetime | None = None) -> str:
...
def _trusted_bundle(
    bundle_type: type[StructuredCommandProfileBundle],
    *,
    bundle_version: str,
    issued_at: datetime,
    expires_at: datetime,
